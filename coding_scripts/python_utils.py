...
This I beleive is correct. leetcode seems to be faulty wiht the output probably 
"""
...
def totalFruit( tree):
    ...
    for idx, fruit in enumerate(tree):
        ...
        basckets.append(fruit)
        
        fruit_types=set(basckets)
        
        if  len(fruit_types)<=2:
            if idx == len(tree)-1:
                maxFruits = maxFruits if maxFruits > len(basckets)  else len(basckets) 
                
        else:
            #if is not in the bascket,find how many types there are 
            
            maxFruits = maxFruits if maxFruits > len(basckets) -1 else len(basckets) -1 # -1 cause the one inserted does not count
            #Since we already added the fruit. look for -2 fruit and find idx of such
            #Need to get the latest index of neither the just inserted nor the prev to the last
            fruit_to_remove = [f for f in fruit_types if f is not basckets[-2] and f is not basckets[-1] ]
            
            idx_f = (len(basckets)-1) -  basckets[::-1].index(fruit_to_remove[0])
            basckets =basckets[idx_f+1:]
